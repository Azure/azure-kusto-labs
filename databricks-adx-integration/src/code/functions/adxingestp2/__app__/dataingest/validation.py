...
# An upper bound on the databases one deployment provisions, so a mistyped count
# cannot ask this function to generate an unbounded number of names.
MAX_DATABASE_COUNT = 100000

# Kusto table names are plain identifiers. The provisioning tool puts them into
# management commands unquoted, so anything else was never created.
_TABLE_NAME_REGEX = re.compile(r'^[A-Za-z][A-Za-z0-9_]{0,1023}$')
...
def build_database_allow_list(name_format: Optional[str], count: Optional[int]) -> FrozenSet[str]:
    ...
    ``name_format``, so applying the same substitution here reproduces the set
    that was created. Both sides read the value from the same deployment
    configuration.

    The result is kept per configuration because the settings are read again on
    every queue message, and generating up to ``MAX_DATABASE_COUNT`` names for
    each one would dominate the cost of ingesting a file. A change to either
    value produces a different key, so the set cannot go stale.
    ...
    """
    if isinstance(count, bool) or not isinstance(count, int) or not 0 < count <= MAX_DATABASE_COUNT:
        raise ValidationError(
    ...
    try:
        ...
        raise ValidationError(
            'ALLOWED_DATABASE_NAME_FORMAT {!r} is not a usable name template.'.format(
                name_format)) from exc
    if len(names) != count:
        raise ValidationError(
            'ALLOWED_DATABASE_NAME_FORMAT {!r} does not name each database once.'.format(
                name_format))
    return names
...
def build_table_allow_list(raw: Optional[str]) -> Set[str]:
    ...
    Split the same way the provisioning tool splits it, so the tables authorised
